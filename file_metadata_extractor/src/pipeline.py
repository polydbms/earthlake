...
class Pipeline:
    ...
    def get_extract_result(self,
        ...
        if mode in config['agent']['mode'].keys(): # 这里的config是config.yaml文件
        ...
        for agent_name, method_name in sorted_process_method.items():
            ...
            # self表示从当前对象获取属性
            # self.schema_agent = SchemaAgent(llm = ChatGPT)
            # self.extraction_agent = ExtractionAgent(llm = ChatGPT, case_repo = self.case_repo)
            if not agent:
                raise AttributeError(f"{agent_name} does not exist.")
            method = getattr(agent, method_name, None) #SchemaAgent.get_retrieved_schema / ExtractionAgent.extract_information_direct
            if not method:
                raise AttributeError(f"Method '{method_name}' not found in {agent_name}.")
            data = method(data) # 主要就是把schema转换成json形式读入，切割文档 / 得到llm的回复列表，一个chunk一个回复
        ...
        data = self.extraction_agent.summarize_answer(data) # 总结所有chunk的回复

        # show result
        if show_trajectory:
            print("Extraction Trajectory: \n", json.dumps(data.get_result_trajectory(), indent=2))
        logprob_result = data.logprob_value
        ...
        # return result
        result = data.pred # 处理完所有chunk后最终的输出
        trajectory = data.get_result_trajectory()

        return result, logprob_result, trajectory, frontend_schema, frontend_res
